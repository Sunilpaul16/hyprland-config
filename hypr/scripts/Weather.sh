...
# Get city from time zone (e.g., Europe/London → London)
city=$(timedatectl show -p Timezone --value | awk -F/ '{print $2}')
[ -z "$city" ] && city="London"  # fallback

cachedir="$HOME/.cache/rbn"
cachefile="${0##*/}-${1:-default}"

mkdir -p "$cachedir"
touch "$cachedir/$cachefile"

# Cache expiry check (29 minutes)
cacheage=$(($(date +%s) - $(date +%s -r "$cachedir/$cachefile")))
if [ $cacheage -gt 1740 ] || [ ! -s "$cachedir/$cachefile" ]; then
  data=($(curl -s "https://en.wttr.in/${city}${1}?0qnT"))
  echo "${data[0]}" | cut -f1 -d, > "$cachedir/$cachefile"
  echo "${data[1]}" | sed -E 's/^.{15}//' >> "$cachedir/$cachefile"
  echo "${data[2]}" | sed -E 's/^.{15}//' >> "$cachedir/$cachefile"
fi

IFS=$'\n' read -d '' -r -a weather < "$cachedir/$cachefile"
