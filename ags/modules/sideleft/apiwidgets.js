...
import {
  BooruView,
  booruCommands,
  sendMessage as booruSendMessage,
  booruTabIcon,
} from "./apis/booru.js";
    ...
    self
        ...
        (self) => {
          ...
          self.placeholderText =
            GPTService.key.length > 0
              ? getString("Message the model...")
              : getString("Enter API Key...");
        },
        "hasKey"
      )
      .hook(
        ...
        (self) => {
          ...
          self.placeholderText =
            Gemini.key.length > 0
              ? getString("Message Gemini...")
              : getString("Enter Google AI API Key...");
        },
        "hasKey"
      )
      .on("key-press-event", (widget, event) => {
        ...
        if (
          ...
          event.get_keyval()[1] === Gdk.KEY_KP_Enter
        ) {
...
let currentApiId = APIS.findIndex(
  (obj) => obj.id === userOptions.sidebar.pages.apis.defaultPage
);
...
chatEntry.get_buffer().connect("changed", (buffer) => {
  ...
  if (
    ...
    bufferText.length > EXPAND_INPUT_THRESHOLD
  ) {
